...
void print_sets(const std::vector<std::vector<addr_tuple>>& sets_array, uint64_t flags, size_t rounds); 
    ...
    for (size_t bits = 2L; bits <= max_fn_bits; bits++) {
        ...
        while (fn_mask <= last_mask) {
            ...
            for (size_t idx = 0; idx<sets.size(); idx++) {
                ...
                if (num_0 < 5)
                {
                    verbose_printerr("\t\t[ NUM_0-%d ]--[ NUM_1-%d ]<============================MARK!!!\n", num_0, num_1);
                }
                else if (num_1 < 5)
                {
                    verbose_printerr("\t\t[ NUM_1-%d ]--[ NUM_0-%d ]<============================MARK!!!\n", num_1, num_0);
                }
                else
                {
...
void rev_mc(size_t sets_cnt, size_t threshold, size_t rounds, size_t m_size, char* o_file, uint64_t flags) {    
    ...
    // 构建一个数据结构存储所有的银行输出文件路径
    std::vector<const char*> bank_output_files = {
        "bank0.csv", "bank1.csv", "bank2.csv", "bank3.csv",
        "bank4.csv", "bank5.csv", "bank6.csv", "bank7.csv",
        "bank8.csv", "bank9.csv", "banka.csv", "bankb.csv",
        "bankc.csv", "bankd.csv"
    };
    ...
    if (flags & F_EXPORT) {
        ...
        if((o_fd = open(o_file, O_CREAT|O_TRUNC|O_RDWR, 0644)) == -1) { // Added O_TRUNC and permissions
    ...
    // 循环遍历文件路径并写入文件头
    if (flags & F_EXPORT) {
    	for (const char* bank_file_name : bank_output_files) {
            int bank_fd = open(bank_file_name, O_CREAT|O_TRUNC|O_RDWR, 0644); 
            if (bank_fd == -1) {
            	perror("[ERROR] - Unable to create bank export file");
            	exit(1); // 如果任何文件无法创建，则退出
            }
            dprintf(bank_fd, O_HEADER); // 写入文件头
            close(bank_fd); // 关闭文件
    	}
    }
    ...
    if (flags & F_VERBOSE) {
	print_sets(sets, flags, rounds);
    }

    if (flags & F_EXPORT) {
	int idx = 0;
    	for (const char* bank_file_name : bank_output_files) {
            int bank_fd = open(bank_file_name, O_CREAT|O_RDWR, 0644); 
            if (bank_fd == -1) {
            	perror("[ERROR] - Unable to create bank export file");
            	exit(1); // 如果任何文件无法创建，则退出
            }
            const addr_tuple& base_addr = sets[idx][0];

	    for (size_t i = 0; i < sets[idx].size(); ++i) { // 遍历集合中的所有地址
                const auto& tmp = sets[idx][i];
                uint64_t latency = 0;
                if (i == 0) { // 基地址与自身的时延可认为是0
                    latency = 0; 
                } else {
                    // 计算当前地址与基地址之间的访问时延
                    latency = time_tuple((volatile char*)base_addr.v_addr, (volatile char*)tmp.v_addr, rounds);
                }
                dprintf(bank_fd, "%lx,%lx,%ld\n",(uint64_t) base_addr.v_addr, (uint64_t) tmp.v_addr,latency);
            }
            close(bank_fd); // 关闭文件
	    idx++;
    	}
    }
    // 在进行 bank 掩码破解前，提示用户敲击回车
    if (flags & F_VERBOSE) {
    ...
    fn_masks = find_functions(sets, 2, 21, flags);
...
//----------------------------------------------------------
// 用于输出不同Row集合的地址对
void print_sets(const std::vector<std::vector<addr_tuple>>& sets_array, uint64_t flags, size_t rounds) { // 增加了 rounds 参数

    for (int idx = 0; idx < 16; idx++) {
        if (!sets_array[idx].empty()) {
            verbose_printerr("[LOG] - Bank %d\tSize: %ld\n", idx, sets_array[idx].size());    
            // 获取当前集合的基地址
            const addr_tuple& base_addr = sets_array[idx][0];
            verbose_printerr("\tBase VAddr: %p - Base PAddr: %p\n", base_addr.v_addr, (void*) base_addr.p_addr);

            for (size_t i = 0; i < sets_array[idx].size(); ++i) { // 遍历集合中的所有地址
                const auto& tmp = sets_array[idx][i];
                uint64_t latency = 0;
                if (i == 0) { // 基地址与自身的时延可认为是0
                    latency = 0; 
                } else {
                ...
                verbose_printerr("\tVAddr:%p - PAddr:%p - Latency to base: %lu cycles\n", tmp.v_addr, (void*) tmp.p_addr, latency);
            }
        }
    }    
}
